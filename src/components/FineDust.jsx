...
export default function FineDust({ airQualityStatus, fineDust }) {
    ...
    <div css={styleCard}>
      ...
      <div css={styleFindDustBody}>
        <figure css={styleFineDustImageWrapper}>
          <img
            src={fineDustInfo[airQualityStatus].image}
            alt="water-icon"
            css={styleFineDustImage}
          />
        </figure>
        <span css={[styleCardTitle, styleFineDustNumber]}>{fineDust}</span>
      </div>
      <p css={[styleCardTitle, styleCardContent]}>
        {fineDustInfo[airQualityStatus].content}
      </p>
    </div>
  );
}
